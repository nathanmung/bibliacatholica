...
# Dossier des .txt
txt_folder = 'txts'
max_chapters = 36  # Full Nombres

for chapter_num in range(1, max_chapters + 1):
  filename = f'{txt_folder}/nombres_chap{chapter_num}.txt'
  ...
  for line in lines:
    line = line.strip()
    if line.startswith("Livre des Nombres") or not line or not line[0].isdigit():  # Skip titre/vide
      continue
...
# Sauve JSON full
output_filename = 'nombres_full_aelf.json'
with open(output_filename, 'w', encoding='utf-8') as f:
  json.dump({"book": book_info}, f, ensure_ascii=False, indent=4)

print(f"JSON full Nombres généré : {output_filename} (avec {len(book_info['chapters'])} chapitres)")